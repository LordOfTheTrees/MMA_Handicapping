...
| **Validation tooling** | [`src/cli/chart_elo_distributions.py`](../src/cli/chart_elo_distributions.py) (`python -m src.cli.chart_elo_distributions`) — histograms by division, summary stats, **`--top-n`** ranked tables; combined chart **`data/elo_by_division.png`**; per-division PNGs **`data/figures/division_elo_histograms/`** (see **`--no-per-division-charts`**). |
