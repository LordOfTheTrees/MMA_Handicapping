...
| [`scripts/export_artifacts.py`](../scripts/export_artifacts.py) | Pickle to `model_weights.json`, `elo_states.json`, `style_axes.json`, `fighter_profiles.json`, `reference_distributions.json`, `espn_crosswalk.json` (from `--data-dir`), `feature_interpretability.json` |
...
**GitHub Actions:** **[`.github/workflows/weekly-model-refresh.yml`](../.github/workflows/weekly-model-refresh.yml)** and **`monthly-model-retrain.yml`** produce **`JSON_exports/`** inside artifacts **`weekly-refresh-<run_id>`** / **`monthly-retrain-<run_id>`**. **[`sync-json-to-mma-ai.yml`](../.github/workflows/sync-json-to-mma-ai.yml)** runs when those complete successfully on the default branch (or on manual dispatch; **`run-bundle`** = newest matching artifact) and pushes JSON into **`mma.ai`** using secret **`MMA_AI_SYNC_PAT`**. Operator/admin notes (**`allow_stale_data`**, scrape failures): **[`docs/BACKEND_PIPELINE_INTEGRATION.md`](BACKEND_PIPELINE_INTEGRATION.md)** § GitHub Actions admin.
