...
Populate **`data/upcoming_cards.json`** with **`python -m src.data.ufcstats_upcoming --data-dir ./data`** or via **`refresh_data()`** (**`train --full-rebuild`**). Checklist vs **`mma.ai`**: **`docs/BACKEND_PIPELINE_INTEGRATION.md`**. **Harness:** **`python scripts/run_harness.py`** (`quick`, `integration`, or default full discover). Snapshot inference [`src/export/json_inference.py`](../src/export/json_inference.py) reproduces **`predict_proba_point_only`** from the four inference JSONs when **`fight_date` == `as_of_date`**. **`data/model.pkl`** vs **`MMA_HARNESS_MODEL`**: **`docs/BACKEND_PIPELINE_INTEGRATION.md`** (Harness).
