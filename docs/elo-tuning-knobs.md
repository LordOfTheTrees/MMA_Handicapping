...
- [`src/cli/chart_elo_distributions.py`](../src/cli/chart_elo_distributions.py) (`python -m src.cli.chart_elo_distributions`) — visualize per-division ELO spread after changes (`--top-n` for ranked tables); combined chart `data/elo_by_division.png` (existing file is deleted then rewritten each run); individual division PNGs default to `data/figures/division_elo_histograms/`
