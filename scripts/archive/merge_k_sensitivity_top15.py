...
From repo root (default input under ``docs/``)::

    python scripts/archive/merge_k_sensitivity_top15.py
    python scripts/archive/merge_k_sensitivity_top15.py path/to/comparison.txt
"""
...
def main() -> None:
    src = Path(__file__).resolve().parents[2] / "docs" / "k sensitivity comparison top 15.txt"
    if len(sys.argv) > 1:
        src = Path(sys.argv[1])
